...
def extract_matches(df, season_key):
    ...
    for index, row in df.iterrows():
        ...
        if is_date:
            ...
            # --- 1. DETERMINE HOME/AWAY STATUS ---
            tamarindi_is_home = False
            opponent = 'Unknown'
            
            # Check Column 2 for Tamarindi (Home)
            ...
            # Check Column 5 (or 6) for Tamarindi (Away)
            ...
            # If the team name is not explicitly found (e.g. older files)
            else:
                # Assume Away by default and opponent is Col 2 (Safest default based on files)
                tamarindi_is_home = False
                opponent = str(row[2]).strip() if not pd.isna(row[2]) else 'Unknown'

            # --- 2. EXTRACT SCORE AND CALCULATE RESULT ---
            ...
            if len(score_parts) == 2 and score_parts[0].strip().isdigit() and score_parts[1].strip().isdigit():
                ...
                # Determine TFC's score based on position
                if tamarindi_is_home:
                    tamarindi_score = home_score
                    opponent_score = away_score
                else:
                    tamarindi_score = away_score
                    opponent_score = home_score

                # Calculate Result
                if tamarindi_score > opponent_score:
                    result = 'W'
                elif tamarindi_score < opponent_score:
                    result = 'L'
                else:
                    result = 'D'
        ...
        # 3. Look for Scorers (Logic simplified for readability and robustness)
        elif current_match:
            # Check Col 2 (Home Team column) and Col 5 (Opponent Column)
            ...
            # Scorers usually appear in the column of the team that scored
            if current_match['home_status'] == 'Home' and potential_scorer_home and not any(word in potential_scorer_home for word in ['Tamarindi', 'FC', 'Club', 'Torneo']):
                 scorer = potential_scorer_home
            elif current_match['home_status'] == 'Away' and potential_scorer_away and not any(word in potential_scorer_away for word in ['Tamarindi', 'FC', 'Club', 'Torneo']):
